...
const CoursesPage = () => {
  ...
  return (
    <main className="pt-36 pb-20 min-h-screen bg-slate-50 relative overflow-hidden">
      ...
      {/* Page Header */}
      <div className="section-container text-center max-w-4xl mx-auto mb-24 animate-[fadeUp_0.8s_ease_forwards]">
      ...
      <div className="section-container mb-32 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {certificationCourses.map((course, idx) => (
            <div key={idx} className="bg-white rounded-[2rem] overflow-hidden shadow-[0_15px_30px_rgba(0,0,0,0.02)] border border-slate-100 group hover:shadow-[0_30px_60px_rgba(15,23,42,0.1)] hover:-translate-y-2 transition-all duration-500">
              <div className="h-64 overflow-hidden relative">
                ...
                />
                <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-bold text-slate-800 shadow-sm border border-slate-200/50">
                  {course.duration}
                </div>
              </div>
              <div className="p-8">
                ...
                <p className="text-slate-500 text-sm leading-relaxed mb-8 line-clamp-4 font-semibold">{course.desc}</p>
                <a 
                  href="https://wa.me/919846067770" 
                  target="_blank" 
                  rel="noreferrer"
                  className="block w-full py-4 text-center rounded-2xl bg-slate-50 border border-slate-100 font-bold text-sm text-brand-dark group-hover:bg-brand-lime group-hover:border-brand-lime group-hover:text-brand-dark transition-all duration-300"
                >
                  Apply & Get Details
                </a>
              </div>
        ...
        <div className="section-container relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {degreePrograms.map((degree, idx) => (
              <div key={idx} className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 md:p-12 hover:bg-white/10 hover:border-brand-lime/30 transition-all duration-500 group flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start mb-8">
                    ...
                    <span className="bg-white/10 px-4 py-1.5 rounded-full text-xs font-bold text-slate-300 tracking-wider uppercase">
                      {degree.tag}
                    </span>
                  </div>
